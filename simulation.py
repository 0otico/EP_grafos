...
def sim(Tritmo, Tlimiar, Tfiltro, Tfinal, Tgrafo, Tcolor): #as tres variaveis de tempo, o tempo final, e o grafo a ser utilisado
    ct = 0 #tempo inicial
    ...
    for x in range(T):
        p = popu.addI(p, ind.new(col.new(Tgrafo, Tcolor), ct, gen))
        gen += 1
    ...
    while Tfinal >= ct:
        ...
        if event.kind(e) == "avaliacao": #se o proximo evento for avaliacao
            ...
            if random() <= (1-(2/pi)*math.atan((1+A)**(1+8*(1+I)))): # a formula dame um valor de 0 a 1, que sera a probabilidade de ocorrer morte, se o numero aleatorio de 0 a 1 for menor ou igual a esse valor, ele morre.
                T -= 1
                p = popu.kill(p,idx) #kills the poor guy
                c = cap.delete_id(c, idx)
        ...
        elif event.kind(cap.nextE(c)) == "evolucao":
            ...
            while ind.coef(popu.worst(p)) < 1: #deleta os invalidos
                c = cap.delete_id(c, popu.worst(p))
        ...
        event.delete(cap.nextE(c)) #acabou o evento atual, passamos para o proximo 
        ct = event.date(cap.nextE(c)) #data do proximo evento